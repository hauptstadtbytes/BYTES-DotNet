...
{
    /// <summary>
    /// the (default) extensions source implementation
    /// </summary>
    [Serializable]
    [XmlType("Source")]
    public class ExtensionsSource : IExtensionsSource
    {
        ...
        public ExtensionsSource()
        {
        }

        /// <summary>
        /// overloaded new instance method, supporting source and pattern
        /// </summary>
        /// <param name="source"></param>
        /// <param name="pattern"></param>
        public ExtensionsSource(string source, string pattern = null)
        {
            _source = source;
        ...
        #region public method(s) implementing 'IExtensionsSource'
        ...
        public string[] AssemblyPaths(Dictionary<string, string> variables = null)
        {
            ...
            {
                foreach (string assembly in Helper.ExpandWildcardPath(path, variables))
                {
                    output.Add(assembly);
                }
            }
        ...
        public bool ValidateExtensionMetadata<TInterface, TMetadata>(Lazy<TInterface, TMetadata> extension, string pattern = null)
        {
            ...
            foreach (string pat in patterns)
            {
                ...
                {
                    _log.AddEntry("Failed to validate metadata: " + ex.Message, LogEntry.InformationLevel.Exception, ex);
                }
            ...
            foreach (string path in AssemblyPaths(variables)) //loop for each assembly path parsed
            {
                ...
                try //try to load the assembly file and iterate over the types
                {
                    ...
                    foreach (System.Type type in GetImplementations<TInterface>(path))
                    {
                        ...
                        {
                            _log.AddEntry("Failed to create lazy instance of '" + type.ToString() + "' at '" + path + "'", LogEntry.InformationLevel.Warning, ex);
                        }
                ...
                {
                    _log.AddEntry("Failed to load extensions from '" + path + "'", LogEntry.InformationLevel.Exception, ex);
                }
