...
public class DapperCli
{
    public static void Main(string[] args)
    {
        ...
        db.Execute(
            "INSERT INTO Songs (Name, Artist, Album, DurationInSeconds) VALUES (@Name, @Artist, @Album, @DurationInSeconds)",
            songs);
        Console.WriteLine($"{songs.Count} Songs eingefügt.");
                
        foreach (var song in db.Query<Song>("SELECT * FROM Songs"))
        {
    ...
    private static IDbConnection ConnectToDB(string provider, string host, string port, string database, string user, string password)
    {
        IDbConnection db = provider switch
        {
            ...
            "mssql" => new SqlConnection($"Server={host},{port};Database={database};User Id={user};Password={password};TrustServerCertificate=True"),
